...
- **One-off task**: Use the textbox on the system to enter your prompt directly.
- **Iterative task**: Use both the website and the VS Code chat to develop iteratively. See [SETUP.md: Test run the extensions and Copilot](SETUP.md#test-run-the-extensions-and-copilot).
    - Open the corresponding task folder as your workspace in VS Code.
    ...
    - Prompt the Copilot to complete the task, your prompts will be automaticaly logged.
- **Iterative Chatbot tasks**: The repo includes a chatbot starter code used for the **Outline Assistant** and **TripAdvisor** tasks. See [SETUP.md: Test run the chatbot](SETUP.md#test-run-the-chatbot).
    ...
    - Open a terminal in your VSCode workspace and launch the chatbot
        ...
        ```
    - Open a browser window at **http://127.0.0.1:5000**. Choose the e.g., Outline Assistant chatbot according to your task assignment. The chatbot uses the system prompt from e.g., `outlineassistant/prompt.md`.
    - Open [GitHub Copilot Chat](https://learn.microsoft.com/en-us/visualstudio/ide/visual-studio-github-copilot-chat?view=visualstudio#ask-questions-in-the-chat-window) coding agent by selecting `View` > `GitHub Copilot Chat` or clicking the Copilot icon inside VS Code.
    - Interact with Copilot to build your chatbot prompt in e.g., `outlineassistant/prompt.md`.
